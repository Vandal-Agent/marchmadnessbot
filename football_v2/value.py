...
def consensus_probability(contract: KalshiContract, game: SportsbookGame) -> tuple[float | None, int]:
    ...
    for market in game.markets:
        if market.market_type != contract.market_type or len(market.outcomes) != 2: continue
        scores = [team_similarity(contract.target_team, outcome.name) for outcome in market.outcomes]
        if max(scores) < 0.76: continue
        index = scores.index(max(scores)); target, other = market.outcomes[index], market.outcomes[1-index]
        if contract.market_type == "spread":
            if contract.line is None or target.point is None or abs(target.point + contract.line) > 0.01: continue
        target_p, other_p = american_implied_probability(target.price), american_implied_probability(other.price)
        if target_p + other_p > 0: probabilities.append(target_p / (target_p + other_p))
...
def compare_contract(contract: KalshiContract, game: SportsbookGame, match_score: float,
    ...
    if contract.yes_ask is None or not 0 < contract.yes_ask < 1: return None
